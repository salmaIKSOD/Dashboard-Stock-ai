...
import React, { useState, useRef, useEffect } from 'react';
import { useNavigate, useLocation } from 'react-router-dom';
import { Menu, X, UserCircle2, Database, Settings, LogOut, Users } from 'lucide-react';
import { useAuth } from '../context/AuthContext';

const PAGE_META = {
  '/gestion-bases':    { title: 'Gestion des bases',    icon: Database    },
  '/profil':           { title: 'Mon profil',            icon: UserCircle2 },
  '/gestion-comptes':  { title: 'Gestion des comptes',  icon: Users       },
  '/parametres':       { title: 'Paramètres',            icon: Settings    },
};
...
export default function HeaderG({ sidebarOpen, onToggleSidebar }) {
  ...
  // Infos utilisateur depuis AuthContext
  ...
  // Badge rôle : "Administration" pour admin, "Employé" pour employé
  const isAdmin    = user?.Role === 'admin';
  const roleLabel  = isAdmin ? 'Administration' : 'Employé';
  const roleBg     = isAdmin ? 'rgba(18,166,224,0.08)'  : 'rgba(1,168,46,0.08)';
  const roleBorder = isAdmin ? 'rgba(18,166,224,0.20)'  : 'rgba(1,168,46,0.20)';
  const roleColor  = isAdmin ? '#12a6e0'                : '#01a82e';
    ...
    <>
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', position: 'relative' }}>
          ...
          {/* Badge rôle — dynamique selon l'utilisateur connecté */}
          <div className="flex items-center gap-1.5 rounded-full py-1 px-2.5"
            style={{ background: roleBg, border: `1px solid ${roleBorder}` }}>
            <Database size={11} style={{ color: roleColor }} />
            <span style={{ color: roleColor, fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
              {roleLabel}
            </span>
          </div>
          ...
          <div style={{ position: 'relative' }} ref={userRef}>
            ...
            {userOpen && (
              <div style={{
                ...
                {/* Header — vrai nom + email */}
                <div style={{ padding: '0.75rem 1rem', borderBottom: '1px solid #f0f0f0' }}>
                  <p style={{ color: '#0d0c0c', fontSize: '0.75rem', fontWeight: 500, margin: 0 }}>{displayName}</p>
                  <p style={{ color: '#c5c5c5', fontSize: '0.625rem', margin: '2px 0 0' }}>{displayEmail}</p>
                </div>
                ...
                <button
                  style={{ ...menuItemStyle, color: '#e53935', borderTop: '1px solid #f0f0f0' }}
                  onClick={async () => { await logout(); navigate('/login'); setUserOpen(false); }}
                  onMouseEnter={e => { e.currentTarget.style.background = 'rgba(229,57,53,0.05)'; e.currentTarget.style.color = '#c62828'; }}
                  onMouseLeave={e => { e.currentTarget.style.background = 'none'; e.currentTarget.style.color = '#e53935'; }}>
                  <LogOut size={13} /> Déconnexion
                </button>
