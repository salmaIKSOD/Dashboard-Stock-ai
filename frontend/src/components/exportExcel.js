...
function buildRecapSheet(wb, data, cols) {
  ...
  artEntries.forEach(([artCode, artRows], idx) => {
    ...
    // Stock final = valeur de la ligne ayant la date la plus récente
    const rowsWithStock = artRows.filter(r => r[cols.stockFinal] != null);
    rowsWithStock.sort((a, b) => new Date(b[cols.date]) - new Date(a[cols.date]));
    const stockFinal = rowsWithStock.length > 0 ? Number(rowsWithStock[0][cols.stockFinal]) : null;
    const lastDate   = rowsWithStock.length > 0 ? rowsWithStock[0][cols.date] : null;
