 ...
 const handleFilter = async (params) => {
  ...
  return (
    <>
      ...
      {loadingChunks && (
        <div className="bg-white border border-[#e8f4fb] rounded-xl px-5 py-3 flex items-center gap-4 shadow-[0_1px_6px_rgba(18,166,224,0.08)]">
          <div className="flex-1">
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-[#0b7db0] text-[0.75rem] font-semibold">
                Chargement des données…
              </span>
              <span className="text-[#c5c5c5] text-[0.6875rem] font-mono">
                Tranche {loadingChunks.done} / {loadingChunks.total}
              </span>
            </div>
            <div className="h-1.5 bg-[#e8f4fb] rounded-full overflow-hidden">
              <div
                className="h-full bg-gradient-to-r from-[#12a6e0] to-[#0d8fc4] rounded-full transition-all duration-500"
                style={{ width: `${(loadingChunks.done / loadingChunks.total) * 100}%` }}
              />
            </div>
          </div>
        </div>
      ...
      {/* Barre de progression chargement par tranches */}
