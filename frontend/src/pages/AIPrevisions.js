...
//       <Info size={12} className="text-[#c5c5c5] hover:text-[#12a6e0] transition-colors" />
//       {show && (
//         <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 z-50 w-56 px-3 py-2 rounded-lg text-xs text-white leading-relaxed shadow-xl"
//           style={{ background: '#1a1a2e', whiteSpace: 'normal' }}>
//           {text}
//           <span className="absolute top-full left-1/2 -translate-x-1/2 border-4 border-transparent" style={{ borderTopColor: '#1a1a2e' }} />
//         </span>
...
//         <p className="text-[12px] text-[#555] m-0 leading-relaxed">{description}</p>
//         {tip && (
//           <p className="text-[11px] text-[#888] m-0 mt-1.5 flex items-center gap-1">
//             <Info size={11} className="flex-shrink-0" style={{ color }} />
//             {tip}
//           </p>
//         )}
//       </div>
...
//   if (!value || isNaN(value)) return <span className="text-[#c5c5c5] text-xs">N/A</span>;
//   const v = Number(value);
//   const color = v < 30 ? { bg: '#EAF3DE', text: '#27500A' }
//     : v < 60 ? { bg: '#FAEEDA', text: '#633806' }
//     : { bg: '#FCEBEB', text: '#791F1F' };
//   return <span className="text-xs font-semibold px-2 py-0.5 rounded-full" style={{ background: color.bg, color: color.text }}>{v.toFixed(1)}%</span>;
// }

// function RuptureBadge({ value }) {
//   if (!value || isNaN(value)) return <span className="text-[#c5c5c5] text-xs">—</span>;
//   const v = Number(value);
//   const color = v < 7 ? { bg: '#FCEBEB', text: '#791F1F' }
//     : v < 30 ? { bg: '#FAEEDA', text: '#633806' }
//     : { bg: '#EAF3DE', text: '#27500A' };
//   return <span className="text-xs font-semibold px-2 py-0.5 rounded-full" style={{ background: color.bg, color: color.text }}>{v.toFixed(0)}j</span>;
// }
...
//           <button key={p} onClick={() => onChange(p)}
//             className="w-8 h-8 rounded-lg text-[0.75rem] transition-all cursor-pointer"
//             style={{ background: p === page ? '#12a6e0' : '#f5f5f5', color: p === page ? 'white' : '#666', border: p === page ? 'none' : '1px solid #e0e0e0', fontWeight: p === page ? 500 : 400 }}>
//             {p}
//           </button>
//         ))}
//         <button onClick={() => onChange(Math.min(totalPages, page + 1))} disabled={page === totalPages}
...
//   if (loading) return (
//     <div className="flex items-center justify-center py-10 gap-3 text-[#12a6e0]">
//       <Loader2 size={18} className="animate-spin" /><span className="text-sm">Génération des prédictions…</span>
//     </div>
//   );
//   if (error) return <div className="py-6 text-center text-sm text-[#b71c1c]">Erreur : {error}</div>;
//   if (!data) return null;
...
//         ))}
//         <line x1={toX(nTrain)} y1={PT} x2={toX(nTrain)} y2={H - PB} stroke="#c5c5c5" strokeWidth="1" strokeDasharray="4 3" />
//         <line x1={toX(nTrain + nTest)} y1={PT} x2={toX(nTrain + nTest)} y2={H - PB} stroke="#c5c5c5" strokeWidth="1" strokeDasharray="4 3" />
//         <text x={toX(nTrain) + 4} y={PT + 10} fontSize="9" fill="#aaa">↑ début test</text>
//         <text x={toX(nTrain + nTest) + 4} y={PT + 10} fontSize="9" fill="#aaa">↑ futur</text>
//         <path d={makePath(data.train.values, 0)} fill="none" stroke="#378ADD" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
//         <path d={makePath(data.test.values_real, nTrain)} fill="none" stroke="#3B6D11" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
//         <path d={makePath(data.test.values_pred, nTrain)} fill="none" stroke="#EF9F27" strokeWidth="1.5" strokeDasharray="5 3" strokeLinecap="round" strokeLinejoin="round" />
//         <path d={makePath(data.futur.values, nTrain + nTest)} fill="none" stroke="#E24B4A" strokeWidth="1.5" strokeDasharray="3 2" strokeLinecap="round" strokeLinejoin="round" />
//       </svg>

//       <div className="flex gap-4 flex-wrap text-xs text-[#555]">
//         {[
//           { color: '#378ADD', label: 'Historique (train)', dash: false, desc: 'Données utilisées pour entraîner le modèle' },
//           { color: '#3B6D11', label: 'Réel (test)',        dash: false, desc: 'Ce qui s\'est réellement passé' },
//           { color: '#EF9F27', label: 'Prédit (test)',      dash: true,  desc: 'Ce que le modèle a prédit — à comparer avec le vert' },
//           { color: '#E24B4A', label: 'Futur J+30',         dash: true,  desc: 'Prévision des 30 prochains jours' },
...
//       <div className="rounded-lg px-3 py-2 text-[11px] text-[#555] leading-relaxed"
//         style={{ background: '#f8f8f8', borderLeft: '3px solid #EF9F27' }}>
//         <strong>Comment lire ce graphe :</strong> Si la courbe orange (prédit) suit bien la courbe verte (réel), le modèle est fiable.
//         La courbe rouge montre ensuite la prévision pour les 30 prochains jours — utilisez-la pour planifier vos commandes fournisseurs.
//       </div>
//     </div>
//   );
// }
...
//   if (!top15.length) return null;
//   const maxVal = Math.max(...top15.map(r => Number(r.jours_estimes)), 1);
//   const W = 640, barH = 22, gap = 6, PL = 110, PR = 60, PT = 10;
//   const H = PT + top15.length * (barH + gap);
//   const getColor = (v) => v < 7 ? '#E24B4A' : v < 30 ? '#EF9F27' : '#639922';
...
//         <div className="flex items-center gap-2">
//           <div className="w-2 h-2 rounded-full bg-[#e53935]" />
//           <span className="text-[0.75rem] font-semibold uppercase tracking-wide text-[#0d0c0c]">Top 15 articles — jours avant rupture estimée</span>
//         </div>
//         <span className="text-[11px] text-[#aaa]">Plus la barre est courte = plus urgent</span>
//       </div>
//       <div className="p-4">
//         <svg width="100%" viewBox={`0 0 ${W} ${H}`}>
//           {top15.map((r, i) => {
...
//                   {String(r.AR_Ref).length > 12 ? String(r.AR_Ref).slice(0, 12) + '…' : r.AR_Ref}
//                 </text>
//                 <rect x={PL} y={y} width={bw} height={barH} rx="4" fill={col} opacity="0.85" />
//                 <text x={PL + bw + 6} y={y + barH / 2 + 4} fontSize="11" fill={col} fontWeight="600">{v.toFixed(0)}j</text>
//               </g>
//             );
//           })}
//           <line x1={PL} y1={PT} x2={PL} y2={H} stroke="#e8e8e8" strokeWidth="1" />
...
//           {[{ color: '#E24B4A', label: 'Critique < 7 jours' }, { color: '#EF9F27', label: 'Alerte 7 à 30 jours' }, { color: '#639922', label: 'OK > 30 jours' }].map(({ color, label }) => (
//             <div key={label} className="flex items-center gap-1.5">
...
//               </div>
//               <div className="flex items-center gap-2">
//                 <span className="text-xs font-semibold text-[#0d0c0c]">{s.nb}</span>
//                 <span className="text-[10px] text-[#aaa]">{s.pct}%</span>
//               </div>
//             </div>
//           ))}
...
//   const allY = sample.map(r => Number(r.StockFinal)  || 0);
//   const maxX = Math.max(...allX, 1), maxY = Math.max(...allY, 1);
//   const W = 420, H = 240, PL = 48, PR = 16, PT = 16, PB = 40;
//   const cw = W - PL - PR, ch = H - PT - PB;
//   const tx = (v) => PL + (v / maxX) * cw;
//   const ty = (v) => PT + ch - (v / maxY) * ch;
//   const xTicks = [0, Math.round(maxX * 0.5), Math.round(maxX)];
...
//         <div className="flex items-center gap-2">
//           <div className="w-2 h-2 rounded-full bg-[#e53935]" />
//           <span className="text-[0.75rem] font-semibold uppercase tracking-wide text-[#0d0c0c]">Carte des anomalies — Sorties vs Stock</span>
//         </div>
//         <span className="text-[11px] text-[#aaa]">{sample.length} mouvements analysés</span>
//       </div>
//       <div className="p-4">
...
//             <g key={i}>
//               <line x1={PL} y1={ty(v)} x2={W - PR} y2={ty(v)} stroke="#f0f0f0" strokeWidth="1" />
//               <text x={PL - 4} y={ty(v) + 4} textAnchor="end" fontSize="9" fill="#aaa">{v}</text>
//             </g>
//           ))}
//           {xTicks.map((v, i) => (
//             <g key={i}>
//               <line x1={tx(v)} y1={PT} x2={tx(v)} y2={H - PB} stroke="#f0f0f0" strokeWidth="1" />
//               <text x={tx(v)} y={H - PB + 12} textAnchor="middle" fontSize="9" fill="#aaa">{v}</text>
//             </g>
//           ))}
//           <text x={W / 2} y={H - 2} textAnchor="middle" fontSize="10" fill="#888">← Quantité sortie →</text>
//           <text x={10} y={H / 2} textAnchor="middle" fontSize="10" fill="#888" transform={`rotate(-90, 10, ${H / 2})`}>← Stock →</text>
//           {normales.map((r, i) => (
//             <circle key={i} cx={tx(Number(r.TotalSortie) || 0)} cy={ty(Number(r.StockFinal) || 0)} r="2.5" fill="#378ADD" opacity="0.2" />
//           ))}
//           {anomalies.map((r, i) => (
//             <circle key={i} cx={tx(Number(r.TotalSortie) || 0)} cy={ty(Number(r.StockFinal) || 0)} r="3.5" fill="#E24B4A" opacity="0.7" />
//           ))}
//         </svg>
//         <div className="flex gap-4 mt-1 text-xs text-[#555] flex-wrap">
//           <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-full bg-[#378ADD] opacity-60" />Normal ({normales.length})</div>
//           <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-full bg-[#E24B4A]" />Anomalie ({anomalies.length})</div>
//         </div>
//         <div className="rounded-lg px-3 py-2 text-[11px] text-[#555] leading-relaxed mt-3"
//           style={{ background: '#f8f8f8', borderLeft: '3px solid #E24B4A' }}>
//           <strong>Comment lire :</strong> Chaque point représente un mouvement de stock. Les points <span className="font-semibold text-[#E24B4A]">rouges</span> sont des mouvements que le modèle a jugés anormaux — sorties inhabituellement élevées ou stock suspicieusement bas. Ils méritent une vérification manuelle.
//         </div>
//       </div>
//     </div>
//   );
...
//     <div className="flex flex-col gap-4">
//       <ExplainBanner
//         icon={TrendingUp} color="#12a6e0"
//         title="Prévision des sorties — AutoARIMA"
//         description="Le modèle AutoARIMA analyse les données historiques de chaque article pour prédire les quantités qui sortiront dans les 30 prochains jours. Il détecte automatiquement les tendances, les cycles hebdomadaires et les variations saisonnières."
//         tip="Cliquez sur le bouton « Voir » d'un article pour afficher son graphe de prévision détaillé."
//       />

//       <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
//         <KpiCard label="Modèle"           value="AutoARIMA" color="#185FA5"
//           hint="AutoARIMA (Auto Regressive Integrated Moving Average) — modèle statistique de prévision de séries temporelles." />
//         <KpiCard label="Articles prédits" value={rows.length} color="#0d0c0c"
//           hint="Nombre d'articles représentant 80% des sorties totales (sélection Pareto)." />
//         <KpiCard label="MAE moyen"        value={`${maeM} u/j`} color="#854F0B"
//           hint="MAE = Erreur absolue moyenne en unités par jour. Plus c'est bas, plus le modèle est précis." />
//         <KpiCard label="MAPE moyen"       value={mapeM ? `${mapeM}%` : 'N/A'} color="#3B6D11"
//           hint="MAPE = Erreur en pourcentage. Vert < 30%, Orange 30-60%, Rouge > 60%. N/A si toutes les sorties sont à zéro." />
//       </div>
...
//               <button key={k} onClick={() => { setSortKey(k); setPage(1); }}
//                 className="px-2 py-1 rounded text-xs transition-all cursor-pointer"
//                 style={{ background: sortKey === k ? '#E6F1FB' : '#f5f5f5', color: sortKey === k ? '#185FA5' : '#666', fontWeight: sortKey === k ? 500 : 400 }}>
//                 {k}
//               </button>
//             ))}
//           </div>
...
//               <tr className="border-b border-[#f0f0f0] bg-[#f8f8f8]">
//                 <th className="px-4 py-2.5 text-left text-[0.6875rem] font-semibold uppercase tracking-wide text-[#888]">Article</th>
//                 <th className="px-4 py-2.5 text-right text-[0.6875rem] font-semibold uppercase tracking-wide text-[#888]">
//                   MAE <Tooltip text="Erreur absolue moyenne en unités/jour. Plus c'est bas = plus précis." />
//                 </th>
//                 <th className="px-4 py-2.5 text-right text-[0.6875rem] font-semibold uppercase tracking-wide text-[#888]">
//                   RMSE <Tooltip text="Racine de l'erreur quadratique moyenne. Pénalise davantage les grandes erreurs." />
//                 </th>
//                 <th className="px-4 py-2.5 text-right text-[0.6875rem] font-semibold uppercase tracking-wide text-[#888]">
//                   MAPE <Tooltip text="Erreur en %. Vert < 30% = excellent. Orange = acceptable. Rouge = à améliorer." />
//                 </th>
//                 <th className="px-4 py-2.5 text-right text-[0.6875rem] font-semibold uppercase tracking-wide text-[#888]">
//                   Train <Tooltip text="Nombre de jours utilisés pour entraîner le modèle (80% des données)." />
//                 </th>
//                 <th className="px-4 py-2.5 text-right text-[0.6875rem] font-semibold uppercase tracking-wide text-[#888]">
//                   Test <Tooltip text="Nombre de jours utilisés pour évaluer le modèle (20% des données non vues)." />
//                 </th>
//                 <th className="px-4 py-2.5 text-center text-[0.6875rem] font-semibold uppercase tracking-wide text-[#888]">Graphe</th>
//               </tr>
//             </thead>
...
//                     <button onClick={() => setSelectedArt(selectedArt === r.AR_Ref ? null : r.AR_Ref)}
//                       className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-[0.7rem] font-medium transition-all cursor-pointer"
//                       style={{ background: selectedArt === r.AR_Ref ? '#E6F1FB' : '#f5f5f5', color: selectedArt === r.AR_Ref ? '#185FA5' : '#666', border: selectedArt === r.AR_Ref ? '1px solid #B5D4F4' : '1px solid #e0e0e0' }}>
//                       <BarChart2 size={11} />{selectedArt === r.AR_Ref ? 'Fermer' : 'Voir'}
//                     </button>
//                   </td>
//                 </tr>
//               ))}
...
//           </table>
//         </div>
//         <Pagination page={page} total={rows.length} pageSize={PAGE_SIZE_PROPHET} onChange={setPage} />
//       </div>
//     </div>
//   );
// }
...
//     <div className="flex flex-col gap-4">
//       <ExplainBanner
//         icon={AlertTriangle} color="#e53935"
//         title="Estimation des ruptures de stock — Random Forest"
//         description="Le modèle calcule pour chaque article le nombre de jours restants avant une rupture de stock, en se basant sur le stock actuel et le rythme moyen des sorties sur les 7 derniers jours. C'est une estimation — pas une certitude."
//         tip="Rouge = commander en urgence. Orange = prévoir une commande prochainement. Vert = stock suffisant."
//       />

//       <div className="grid grid-cols-3 gap-3">
//         <KpiCard label="Rupture critique (< 7j)" value={danger} color="#A32D2D"
//           hint="Articles dont le stock va s'épuiser dans moins d'une semaine. Commande urgente requise." />
//         <KpiCard label="Alerte (7 à 30j)"         value={alerte} color="#854F0B"
//           hint="Articles qui seront en rupture dans 1 à 4 semaines. Planifier une commande." />
//         <KpiCard label="Stock OK (> 30j)"          value={ok}     color="#3B6D11"
//           hint="Articles avec un stock suffisant pour plus d'un mois au rythme actuel." />
//       </div>
...
//           <div className="flex items-center gap-2 px-4 py-3 border-b border-[#f0f0f0]">
//             <div className="w-2 h-2 rounded-full bg-[#e53935]" />
//             <span className="text-[0.75rem] font-semibold uppercase tracking-wide text-[#0d0c0c]">Tous les articles — triés par urgence</span>
//             <span className="bg-[#f0f0f0] text-[#666] text-[0.6875rem] font-mono px-2 py-0.5 rounded">{rows.length} articles</span>
//           </div>
//           <div className="overflow-x-auto">
//             <table className="w-full text-sm border-collapse">
...
//                   <th className="px-4 py-2.5 text-left text-[0.6875rem] font-semibold uppercase tracking-wide text-[#888]">Article</th>
//                   <th className="px-4 py-2.5 text-right text-[0.6875rem] font-semibold uppercase tracking-wide text-[#888]">
//                     Jours estimés <Tooltip text="Nombre de jours avant rupture calculé par le modèle Random Forest." />
//                   </th>
//                   <th className="px-4 py-2.5 text-right text-[0.6875rem] font-semibold uppercase tracking-wide text-[#888]">
//                     Stock final <Tooltip text="Dernière quantité connue en stock pour cet article." />
//                   </th>
//                   <th className="px-4 py-2.5 text-right text-[0.6875rem] font-semibold uppercase tracking-wide text-[#888]">
//                     Rythme 7j <Tooltip text="Moyenne des sorties sur les 7 derniers jours. Sert à calculer le nombre de jours restants." />
//                   </th>
//                 </tr>
//               </thead>
//               <tbody>
...
//               </span>
//               <Tooltip text="Ces variables ont le plus influencé les prédictions du modèle Random Forest." />
//             </div>
//             <div className="px-4 py-3 flex flex-col gap-2">
//               {[...importances].sort((a, b) => Number(b.importance) - Number(a.importance)).map((imp, i) => (
//                 <div key={i}>
...
//                   </div>
//                   <div className="h-1.5 bg-[#f0f0f0] rounded-full overflow-hidden">
//                     <div className="h-full rounded-full" style={{ width: `${(Number(imp.importance) / maxImp) * 100}%`, background: '#12a6e0' }} />
//                   </div>
//                 </div>
//               ))}
//             </div>
//             <div className="px-4 pb-3">
//               <div className="rounded-lg px-3 py-2 text-[11px] text-[#555] leading-relaxed"
//                 style={{ background: '#f8f8f8', borderLeft: '3px solid #12a6e0' }}>
//                 Plus le pourcentage est élevé, plus cette variable a pesé dans la décision du modèle.
//               </div>
//             </div>
//           </div>
//         )}
...
//   const pctAno    = ((anomalies.length / rows.length) * 100).toFixed(1);
//   const top100    = [...anomalies].sort((a, b) => Number(a.anomaly_score) - Number(b.anomaly_score)).slice(0, 100);
//   const paginated = top100.slice((page - 1) * PAGE_SIZE_IF, page * PAGE_SIZE_IF);

//   return (
//     <div className="flex flex-col gap-4">
//       <ExplainBanner
//         icon={Radar} color="#EF9F27"
//         title="Détection d'anomalies — Isolation Forest"
//         description="Le modèle Isolation Forest analyse tous les mouvements de stock pour détecter ceux qui sont statistiquement inhabituels : sorties anormalement élevées, stocks suspicieusement bas, ou combinaisons de valeurs rares. Ces mouvements méritent une vérification manuelle."
//         tip="Un score négatif bas (ex: -0.25) = anomalie très sévère. Un score proche de 0 = mouvement limite."
//       />

//       <div className="grid grid-cols-3 gap-3">
//         <KpiCard label="Total analysés" value={fmtNum(rows.length)} color="#0d0c0c"
//           hint="Nombre total de mouvements de stock analysés par le modèle." />
//         <KpiCard label="Anomalies"      value={fmtNum(anomalies.length)} color="#A32D2D"
//           hint="Mouvements jugés anormaux par le modèle. Représentent 5% du total (paramètre contamination)." />
//         <KpiCard label="% anomalies"    value={`${pctAno}%`} color="#854F0B"
//           hint="Pourcentage de mouvements anormaux. Fixé à 5% par défaut — paramètre configurable dans config.py." />
//       </div>
...
//         <div className="flex items-center gap-2 px-4 py-3 border-b border-[#f0f0f0]">
//           <div className="w-2 h-2 rounded-full bg-[#e53935]" />
//           <span className="text-[0.75rem] font-semibold uppercase tracking-wide text-[#0d0c0c]">Anomalies les plus sévères</span>
//           <span className="bg-[#FCEBEB] text-[#791F1F] text-[0.6875rem] font-mono px-2 py-0.5 rounded">
//             score bas = plus sévère <Tooltip text="Le score d'anomalie est négatif. Plus il est bas, plus le mouvement est inhabituel." />
//           </span>
//         </div>
//         <div className="overflow-x-auto">
//           <table className="w-full text-sm border-collapse">
...
//                 <th className="px-4 py-2.5 text-left text-[0.6875rem] font-semibold uppercase tracking-wide text-[#888]">Article</th>
//                 <th className="px-4 py-2.5 text-right text-[0.6875rem] font-semibold uppercase tracking-wide text-[#888]">
//                   Total Sortie <Tooltip text="Quantité sortie ce jour-là pour cet article." />
//                 </th>
//                 <th className="px-4 py-2.5 text-right text-[0.6875rem] font-semibold uppercase tracking-wide text-[#888]">
//                   Stock Final <Tooltip text="Stock restant à la fin de cette journée." />
//                 </th>
//                 <th className="px-4 py-2.5 text-right text-[0.6875rem] font-semibold uppercase tracking-wide text-[#888]">
//                   Score <Tooltip text="Score d'anomalie : plus négatif = plus suspect. Calculé par Isolation Forest." />
//                 </th>
//               </tr>
//             </thead>
//             <tbody>
...
//           </table>
//         </div>
//         <Pagination page={page} total={top100.length} pageSize={PAGE_SIZE_IF} onChange={setPage} />
//       </div>
//     </div>
//   );
// }
...
//   const COLORS = { 'forte rotation': { bg: '#FCEBEB', text: '#791F1F', dot: '#e53935' }, 'rotation moyenne': { bg: '#FAEEDA', text: '#633806', dot: '#EF9F27' }, 'faible rotation': { bg: '#E6F1FB', text: '#0C447C', dot: '#378ADD' }, 'quasi-immobile': { bg: '#F1EFE8', text: '#444441', dot: '#888780' } };
//   const getColor = (seg) => COLORS[seg] || { bg: '#f0f0f0', text: '#555', dot: '#aaa' };
//   const articlesFiltres = filtre ? segments.filter(s => s.segment === filtre) : segments;
//   const paginated       = articlesFiltres.slice((page - 1) * PAGE_SIZE_SEG, page * PAGE_SIZE_SEG);

//   const DESCRIPTIONS = {
//     'forte rotation'   : 'Articles vendus très fréquemment. Priorité maximale pour le réapprovisionnement.',
//     'rotation moyenne' : 'Articles avec une activité régulière. Surveiller le stock régulièrement.',
//     'faible rotation'  : 'Articles peu vendus. Commandes ponctuelles suffisantes.',
//     'quasi-immobile'   : 'Articles presque jamais vendus. Envisager de les liquider ou de réduire le stock.',
//   };

//   return (
//     <div className="flex flex-col gap-4">
//       <ExplainBanner
//         icon={Shapes} color="#7F77DD"
//         title="Segmentation des articles — K-Means"
//         description="Le modèle K-Means regroupe automatiquement les articles selon leur profil de consommation mensuel. Les articles qui se ressemblent dans leur comportement de sortie sont placés dans le même segment. Cela aide à adapter la stratégie de stock à chaque groupe."
//         tip="Utilisez les filtres pour explorer chaque segment et identifier les articles dormants ou les plus actifs."
//       />
...
//             return (
//               <div key={i} className="bg-white border rounded-xl px-4 py-3 flex items-start gap-3" style={{ borderColor: col.dot + '44' }}>
//                 <div className="w-3 h-3 rounded-full flex-shrink-0 mt-1" style={{ background: col.dot }} />
//                 <div>
//                   <p className="text-[12px] font-semibold capitalize m-0" style={{ color: col.text }}>{c.Segment} — {fmtNum(c.Nb_articles)} articles</p>
//                   <p className="text-[11px] text-[#777] m-0 mt-0.5">{desc}</p>
//                 </div>
//               </div>
//             );
...
//             <div className="flex items-center gap-2">
//               <div className="w-2 h-2 rounded-full bg-[#7F77DD]" />
//               <span className="text-[0.75rem] font-semibold uppercase tracking-wide text-[#0d0c0c]">Articles par segment</span>
//               <span className="bg-[#f0f0f0] text-[#666] text-[0.6875rem] font-mono px-2 py-0.5 rounded">{articlesFiltres.length} articles</span>
//             </div>
//             <div className="flex gap-1 flex-wrap">
//               <button onClick={() => setFiltre('')} className="px-2 py-1 text-xs rounded transition-all cursor-pointer"
...
//                 <tr className="border-b border-[#f0f0f0] bg-[#f8f8f8]">
//                   <th className="px-4 py-2.5 text-left text-[0.6875rem] font-semibold uppercase tracking-wide text-[#888]">Article</th>
//                   <th className="px-4 py-2.5 text-left text-[0.6875rem] font-semibold uppercase tracking-wide text-[#888]">
//                     Segment <Tooltip text="Groupe attribué automatiquement par le modèle K-Means selon le profil de consommation mensuel." />
//                   </th>
//                 </tr>
//               </thead>
//               <tbody>
...
//   const banners = {
//     running    : { bg: '#E6F1FB', border: '#B5D4F4', color: '#0C447C', icon: <Loader2 size={15} className="animate-spin flex-shrink-0" />, msg: <>Pipeline en cours pour <strong>{baseName}</strong>… Vérification toutes les 5 secondes.</> },
//     success    : { bg: '#EAF3DE', border: '#C0DD97', color: '#27500A', icon: <CheckCircle size={15} className="flex-shrink-0" />, msg: <>Prédictions disponibles pour <strong>{baseName}</strong>{status?.duration_s ? ` — calculées en ${status.duration_s}s` : ''}</> },
//     failed     : { bg: '#FCEBEB', border: '#F7C1C1', color: '#791F1F', icon: <XCircle size={15} className="flex-shrink-0" />, msg: <>Le pipeline a échoué pour <strong>{baseName}</strong>. Cliquez sur "Relancer" pour réessayer.</> },
//     not_started: { bg: '#FAEEDA', border: '#FAC775', color: '#633806', icon: <Clock size={15} className="flex-shrink-0" />, msg: <>Aucune prédiction générée pour <strong>{baseName}</strong>. Cliquez sur "Relancer" pour lancer le calcul.</> },
//   };
...
//             className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium border transition-all"
//             style={{ background: polling ? '#f5f5f5' : 'white', borderColor: '#e0e0e0', color: polling ? '#aaa' : '#555', cursor: polling ? 'not-allowed' : 'pointer' }}
//             title="Relancer le calcul des prédictions pour cette base">
//             <RefreshCw size={13} className={polling ? 'animate-spin' : ''} />
//             {polling ? 'Calcul en cours…' : 'Relancer'}
//           </button>
//         </div>
...
//       </svg>
//       <div className="flex gap-4 flex-wrap text-xs text-[#555]">
//         {[{ color: '#378ADD', label: 'Historique (train)', dash: false }, { color: '#3B6D11', label: 'Réel (test)', dash: false }, { color: '#EF9F27', label: 'Prédit (test)', dash: true }, { color: '#E24B4A', label: 'Futur J+30', dash: true }].map(({ color, label, dash }) => (
//           <div key={label} className="flex items-center gap-1.5">
//             <svg width="20" height="8"><line x1="0" y1="4" x2="20" y2="4" stroke={color} strokeWidth="2" strokeDasharray={dash ? '4 2' : undefined} /></svg>
//             {label}
//           </div>
...
// function RuptureBarChart({ rows }) {
//   const top15 = [...rows]
//     .filter(r => r.jours_estimes !== null && !isNaN(r.jours_estimes))
...
//         <div className="w-2 h-2 rounded-full bg-[#e53935]" />
//         <span className="text-[0.75rem] font-semibold uppercase tracking-wide text-[#0d0c0c]">
//           Top 15 articles — jours avant rupture
//         </span>
//       </div>
//       <div className="p-4">
//         <svg width="100%" viewBox={`0 0 ${W} ${H}`}>
...
//                 </text>
//                 <rect x={PL} y={y} width={bw} height={barH} rx="4" fill={col} opacity="0.85" />
//                 <text x={PL + bw + 6} y={y + barH / 2 + 4} fontSize="11" fill={col} fontWeight="600">
//                   {v.toFixed(0)}j
//                 </text>
//               </g>
//             );
...
//         </svg>
//         <div className="flex gap-4 mt-2 text-xs text-[#555]">
//           {[{ color: '#E24B4A', label: 'Critique < 7j' }, { color: '#EF9F27', label: 'Alerte 7-30j' }, { color: '#639922', label: 'OK > 30j' }].map(({ color, label }) => (
//             <div key={label} className="flex items-center gap-1.5">
...
//   const segmentColors = {
//     'forte rotation'   : '#E24B4A',
//     'rotation moyenne' : '#EF9F27',
//     'faible rotation'  : '#378ADD',
//     'quasi-immobile'   : '#888780',
//   };
...
//     const color = segmentColors[c.Segment] || '#aaa';
//     return { d: `M ${x1} ${y1} A ${R} ${R} 0 ${large} 1 ${x2} ${y2} L ${xi2} ${yi2} A ${r} ${r} 0 ${large} 0 ${xi1} ${yi1} Z`, color, nb, pct: Math.round(pct * 100), label: c.Segment };
//   });
...
//       <div className="flex items-center gap-2 px-4 py-3 border-b border-[#f0f0f0]">
//         <div className="w-2 h-2 rounded-full bg-[#7F77DD]" />
//         <span className="text-[0.75rem] font-semibold uppercase tracking-wide text-[#0d0c0c]">Répartition par segment</span>
//       </div>
//       <div className="flex items-center gap-6 p-4">
//         <svg width="200" height="200" viewBox="0 0 200 200">
//           {slices.map((s, i) => (
...
//   const W = 400, H = 220, PL = 48, PR = 16, PT = 16, PB = 32;
//   const cw = W - PL - PR, ch = H - PT - PB;
//   const tx = (v) => PL + (v / maxX) * cw;
//   const ty = (v) => PT + ch - (v / maxY) * ch;
...
//         <div className="w-2 h-2 rounded-full bg-[#e53935]" />
//         <span className="text-[0.75rem] font-semibold uppercase tracking-wide text-[#0d0c0c]">
//           Scatter — TotalSortie vs StockFinal
//         </span>
//         <span className="text-[0.6875rem] text-[#aaa]">({sample.length} points)</span>
//       </div>
//       <div className="p-4">
//         <svg width="100%" viewBox={`0 0 ${W} ${H}`}>
//           {yTicks.map((v, i) => (
...
//             </g>
//           ))}
//           <text x={W / 2} y={H - 2} textAnchor="middle" fontSize="10" fill="#aaa">TotalSortie</text>
//           {normales.map((r, i) => (
//             <circle key={i} cx={tx(Number(r.TotalSortie) || 0)} cy={ty(Number(r.StockFinal) || 0)}
//               r="2.5" fill="#378ADD" opacity="0.25" />
//           ))}
//           {anomalies.map((r, i) => (
//             <circle key={i} cx={tx(Number(r.TotalSortie) || 0)} cy={ty(Number(r.StockFinal) || 0)}
//               r="3.5" fill="#E24B4A" opacity="0.7" />
//           ))}
//         </svg>
//         <div className="flex gap-4 mt-1 text-xs text-[#555]">
//           <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-full bg-[#378ADD] opacity-60" />Normal ({normales.length})</div>
//           <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-full bg-[#E24B4A]" />Anomalie ({anomalies.length})</div>
//         </div>
//       </div>
...
//             <thead>
//               <tr className="border-b border-[#f0f0f0] bg-[#f8f8f8]">
//                 {['Article', 'MAE', 'RMSE', 'MAPE', 'Train', 'Test', 'Graphe'].map((h, i) => (
//                   <th key={h} className={`px-4 py-2.5 text-[0.6875rem] font-semibold uppercase tracking-wide text-[#888] ${i === 0 || i === 6 ? 'text-left' : 'text-right'} ${i === 6 ? 'text-center' : ''}`}>{h}</th>
//                 ))}
//               </tr>
//             </thead>
//             <tbody>
...
//           <div className="flex items-center gap-2 px-4 py-3 border-b border-[#f0f0f0]">
//             <div className="w-2 h-2 rounded-full bg-[#e53935]" />
//             <span className="text-[0.75rem] font-semibold uppercase tracking-wide text-[#0d0c0c]">Articles triés par urgence</span>
//             <span className="bg-[#f0f0f0] text-[#666] text-[0.6875rem] font-mono px-2 py-0.5 rounded">{rows.length} articles</span>
//           </div>
//           <div className="overflow-x-auto">
//             <table className="w-full text-sm border-collapse">
//               <thead>
//                 <tr className="border-b border-[#f0f0f0] bg-[#f8f8f8]">
//                   {['Article', 'Jours estimés', 'Stock final', 'Rythme 7j'].map((h, i) => (
//                     <th key={h} className={`px-4 py-2.5 text-[0.6875rem] font-semibold uppercase tracking-wide text-[#888] ${i === 0 ? 'text-left' : 'text-right'}`}>{h}</th>
//                   ))}
//                 </tr>
//               </thead>
//               <tbody>
...
//           <div className="bg-white border border-[#e8e8e8] rounded-xl overflow-hidden">
//             <div className="flex items-center gap-2 px-4 py-3 border-b border-[#f0f0f0]">
//               <span className="text-[0.75rem] font-semibold uppercase tracking-wide text-[#0d0c0c]">Feature importance</span>
//             </div>
//             <div className="px-4 py-3 flex flex-col gap-2">
//               {[...importances].sort((a, b) => Number(b.importance) - Number(a.importance)).map((imp, i) => (
//                 <div key={i}>
...
//           <div className="w-2 h-2 rounded-full bg-[#e53935]" />
//           <span className="text-[0.75rem] font-semibold uppercase tracking-wide text-[#0d0c0c]">Anomalies les plus sévères</span>
//           <span className="bg-[#FCEBEB] text-[#791F1F] text-[0.6875rem] font-mono px-2 py-0.5 rounded">score bas = plus sévère</span>
//         </div>
//         <div className="overflow-x-auto">
//           <table className="w-full text-sm border-collapse">
//             <thead>
//               <tr className="border-b border-[#f0f0f0] bg-[#f8f8f8]">
//                 {['Date', 'Article', 'Total Sortie', 'Stock Final', 'Score'].map((h, i) => (
//                   <th key={h} className={`px-4 py-2.5 text-[0.6875rem] font-semibold uppercase tracking-wide text-[#888] ${i < 2 ? 'text-left' : 'text-right'}`}>{h}</th>
//                 ))}
//               </tr>
//             </thead>
//             <tbody>
...
//     if (!status) return null;
//     const banners = {
//       running : { bg: '#E6F1FB', border: '#B5D4F4', color: '#0C447C', icon: <Loader2 size={15} className="animate-spin flex-shrink-0" />, msg: <>Pipeline en cours pour <strong>{baseName}</strong>… Vérification toutes les 5s.</> },
//       success : { bg: '#EAF3DE', border: '#C0DD97', color: '#27500A', icon: <CheckCircle size={15} className="flex-shrink-0" />, msg: <>Prédictions disponibles pour <strong>{baseName}</strong>{status.duration_s ? ` — durée : ${status.duration_s}s` : ''}</> },
//       failed  : { bg: '#FCEBEB', border: '#F7C1C1', color: '#791F1F', icon: <XCircle size={15} className="flex-shrink-0" />, msg: <>Pipeline échoué pour <strong>{baseName}</strong>. Relancez manuellement.</> },
//       not_started: { bg: '#FAEEDA', border: '#FAC775', color: '#633806', icon: <Clock size={15} className="flex-shrink-0" />, msg: <>Aucune prédiction pour <strong>{baseName}</strong>. Lancez le pipeline.</> },
//     };
//     const key = (status.status === 'running' || polling) ? 'running' : status.status;
//     const b   = banners[key];
//     if (!b) return null;
...
//         <div>
//           <h1 className="text-[1.1rem] font-semibold text-[#0d0c0c] m-0">Analyse prédictive</h1>
//           <p className="text-[0.8rem] text-[#aaaaaa] m-0 mt-0.5">AutoARIMA · Random Forest · Isolation Forest · K-Means</p>
//         </div>
//         <div className="flex items-center gap-2">
//           <BaseSelector bases={bases} value={baseName} onChange={setBaseName} loading={loadingBase} />
//           <button onClick={handleRunPipeline} disabled={!baseName || polling}
