...
function AnomalyScatter({ rows }) {
  ...
  const normales  = normalesAll.length > 500
    ...
    : normalesAll;
  const anomalies = anomaliesAll.slice(0, 150);
...
function TabRuptures({ data }) {
  ...
  // PAGE_SIZE dynamique : aligné sur le nombre de facteurs clés pour éviter le vide
  const dynamicPageSize = importances.length > 0 ? importances.length : PAGE_SIZE;
  const paginated = sorted.slice((page - 1) * dynamicPageSize, page * dynamicPageSize);
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3 items-stretch">
        <div className="lg:col-span-2 bg-white border border-[#e8e8e8] rounded-xl overflow-hidden flex flex-col">
          <div className="flex items-center gap-2 px-4 py-3 border-b border-[#f0f0f0]">
            <div className="w-2 h-2 rounded-full bg-[#e53935]" />
          ...
          <Pagination page={page} total={rows.length} pageSize={dynamicPageSize} onChange={setPage} />
        </div>

        {importances.length > 0 ? (
          <div className="bg-white border border-[#e8e8e8] rounded-xl overflow-hidden flex flex-col">
            <div className="flex items-center gap-2 px-4 py-3 border-b border-[#f0f0f0]">
          ...
          <div className="bg-white border border-[#e8e8e8] rounded-xl p-4 flex flex-col items-center justify-center text-center gap-2 h-full">
