...
// //  + Exports Excel (.xlsx) via exportExcelAlertes.js
// // ══════════════════════════════════════════════════════════════
...
// import { exportExcelAlertes } from '../api/exportExcelAlertes';
...
// function ExportButton({ onClick, label = 'Exporter Excel' }) {
//   return (
//     <button
//       onClick={onClick}
...
// // Nombre de jours de couverture visé à chaque commande
// const JOURS_COUVERTURE = 15;

// // Calcule la quantité suggérée à commander pour couvrir JOURS_COUVERTURE jours de vente
...
//   const quantiteSuggeree = calcQuantiteSuggeree(stock, rythme);
...
//       {/* Quantité suggérée à commander — vise 15 jours de couverture */}
//       {quantiteSuggeree > 0 && (
//         <div className="rounded-lg px-3 py-2 mt-1 flex items-center justify-between gap-2" style={{ background: cfg.dot + '12', border: `1px dashed ${cfg.dot}55` }}>
//           <span className="text-[10px] text-[#666]">Quantité suggérée <span className="text-[9px] text-[#aaa]">(couvre 15 j)</span></span>
//           <span className="text-[0.95rem] font-bold" style={{ color: cfg.dot }}>{fmtNum(quantiteSuggeree)} u.</span>
//         </div>
//       )}
//     </div>
//   );
...
// // ── Sélecteur de base — avec fermeture au clic extérieur ─────────
...
//           </div>
//           {critiques.length > 0 && (
//             <ExportButton onClick={() => exportExcelAlertes(critiques, `commander_maintenant_${baseName}`,
//               ['Article', 'Désignation', 'Temps restant', 'Stock actuel', 'Ventes/jour', 'Quantité suggérée (15j)'],
//               r => [r.AR_Ref, getDesign(r), fmtJoursHeures(r.jours_estimes), Number(r.StockFinal).toFixed(0), Number(r.rolling_mean_7).toFixed(1), calcQuantiteSuggeree(r.StockFinal, r.rolling_mean_7)],
//               { sheetName: 'Commander maintenant', title: `Commander maintenant — ${baseName}`,
//                 subtitle: 'Articles dont le stock s\'épuise en moins de 7 jours', colWidths: [14, 30, 16, 14, 14, 20] })} />
//           )}
//         </div>
//         <div className="p-4">
...
//           </div>
//           {alertes.length > 0 && (
//             <ExportButton onClick={() => exportExcelAlertes(alertes, `planifier_${baseName}`,
//               ['Article', 'Désignation', 'Temps restant', 'Stock actuel', 'Ventes/jour', 'Quantité suggérée (15j)'],
//               r => [r.AR_Ref, getDesign(r), fmtJoursHeures(r.jours_estimes), Number(r.StockFinal).toFixed(0), Number(r.rolling_mean_7).toFixed(1), calcQuantiteSuggeree(r.StockFinal, r.rolling_mean_7)],
//               { sheetName: 'Planifier', title: `Planifier cette semaine — ${baseName}`,
//                 subtitle: 'Articles dont le stock s\'épuise entre 7 et 30 jours', colWidths: [14, 30, 16, 14, 14, 20] })} />
//           )}
//         </div>
//         <div className="p-4">
...
//           </div>
//           {ok.length > 0 && (
//             <ExportButton onClick={() => exportExcelAlertes(ok, `stock_ok_${baseName}`,
...
//               { sheetName: 'Stock OK', title: `Stock suffisant — ${baseName}`,
//                 subtitle: 'Articles avec plus de 30 jours de stock', colWidths: [14, 30, 16, 14, 14] })} />
//           )}
//         </div>
//         <div className="p-4">
...
//           </div>
//           <ExportButton onClick={() => exportExcelAlertes(filtres, `previsions_${baseName}`,
...
//             },
//             { sheetName: 'Prévisions', title: `Prévisions du mois — ${baseName}`,
//               subtitle: 'Quantités estimées à commander pour le mois prochain', colWidths: [14, 30, 18, 14, 35] })} />
//         </div>
...
//           </div>
//           {severes.length > 0 && (
//             <ExportButton onClick={() => exportExcelAlertes(filtres, `anomalies_${baseName}`,
//               ['Date', 'Article', 'Désignation', 'Qté sortie', 'Stock restant', 'Sévérité'],
//               r => [fmtDate(r.DateJour), r.AR_Ref, getDesign(r), Number(r.TotalSortie).toFixed(1), Number(r.StockFinal).toFixed(1),
//                 Math.abs(r.anomaly_score) > 0.2 ? 'Très suspect' : Math.abs(r.anomaly_score) > 0.1 ? 'Suspect' : 'À vérifier'],
//               { sheetName: 'Mouvements suspects', title: `Mouvements suspects — ${baseName}`,
//                 subtitle: 'Sorties inhabituelles détectées automatiquement, à vérifier dans SAGE', colWidths: [14, 14, 30, 14, 16, 16] })} />
//           )}
//         </div>
...
//             <ExportButton onClick={() => exportExcelAlertes(prioritaires, `articles_prioritaires_${baseName}`,
//               ['Article', 'Désignation', 'Segment', 'Conseil'],
//               r => [r.AR_Ref, getDesign(r), r.segment, 'Commander souvent et en grande quantité'],
//               { sheetName: 'Prioritaires', title: `Articles prioritaires — ${baseName}`,
//                 subtitle: 'Articles à forte rotation — réapprovisionnement prioritaire', colWidths: [14, 30, 18, 40] })} />
//           )}
//         </div>
//         {prioritaires.length === 0 ? (
...
// //               r => [r.AR_Ref, getDesign(r), fmtJoursHeures(r.jours_estimes), Number(r.StockFinal).toFixed(0), Number(r.rolling_mean_7).toFixed(1)],
// //               { sheetName: 'Commander maintenant', title: `Commander maintenant — ${baseName}`,
// //                 subtitle: 'Articles dont le stock s\'épuise en moins de 7 jours', colWidths: [14, 30, 16, 14, 14] })} />
// //           )}
// //         </div>
// //         <div className="p-4">
// //           {critiques.length === 0 ? <EmptyState msg="Aucun article en rupture critique." /> : (
...
// //               r => [r.AR_Ref, getDesign(r), fmtJoursHeures(r.jours_estimes), Number(r.StockFinal).toFixed(0), Number(r.rolling_mean_7).toFixed(1)],
// //               { sheetName: 'Planifier', title: `Planifier cette semaine — ${baseName}`,
// //                 subtitle: 'Articles dont le stock s\'épuise entre 7 et 30 jours', colWidths: [14, 30, 16, 14, 14] })} />
// //           )}
// //         </div>
// //         <div className="p-4">
// //           {alertes.length === 0 ? <EmptyState msg="Aucun article en alerte." /> : (
...
//         <div className="p-4">
//           {paginated.length === 0 ? (
//             <p className="text-center text-[#aaa] text-sm py-8">Aucun article dans cette catégorie</p>
//           ) : (
//             <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
//               {paginated.map((r, i) => {
//                 const mape = Number(r.MAPE);
//                 const fiable = isNaN(mape) ? null : mape < 30 ? 'Très fiable' : mape < 60 ? 'Fiable' : 'Peu fiable';
//                 const fiableColor = isNaN(mape) ? '#888' : mape < 30 ? '#15803d' : mape < 60 ? '#854F0B' : '#A32D2D';
//                 const fiableBg = isNaN(mape) ? '#f5f5f5' : mape < 30 ? '#F0FDF4' : mape < 60 ? '#FFFBEB' : '#FFF5F5';
//                 const fiableBorder = isNaN(mape) ? '#e0e0e0' : mape < 30 ? '#86EFAC' : mape < 60 ? '#FCD34D' : '#FCA5A5';
//                 const conseil = isNaN(mape) ? 'Vérifier les données' : mape < 30 ? 'Utiliser cette prévision' : mape < 60 ? 'Prévision indicative' : 'Commander selon expérience';
//                 return (
//                   <div key={i} className="rounded-xl border p-4 flex flex-col gap-2" style={{ background: fiableBg, borderColor: fiableBorder }}>
//                     <div className="flex items-center justify-between gap-2 flex-wrap">
//                       <span className="font-mono text-[0.75rem] font-semibold text-[#0b7db0] bg-white border border-[#B5D4F4] rounded px-2 py-0.5 flex-shrink-0">{r.AR_Ref}</span>
//                       <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full flex-shrink-0" style={{ background: fiableColor + '18', color: fiableColor }}>{fiable || 'N/A'}</span>
//                     </div>
//                     <p className="text-[0.8rem] text-[#444] m-0 font-medium truncate" title={getDesign(r)}>{getDesign(r)}</p>
//                     <div className="bg-white rounded-lg px-3 py-2 border border-[#f0f0f0] flex items-center justify-between">
//                       <span className="text-[10px] text-[#aaa]">Quantité prévue</span>
//                       <span className="text-[0.95rem] font-bold" style={{ color: fiableColor }}>{fmtNum(r.MAE)} u./jour</span>
//                     </div>
//                     <p className="text-[0.78rem] text-[#555] m-0">{conseil}</p>
//                   </div>
//                 );
//               })}
//             </div>
...
//             <div className="p-4">
//               {paginated.length === 0 ? (
//                 <p className="text-center text-[#aaa] text-sm py-8">Aucun mouvement dans cette catégorie</p>
//               ) : (
//                 <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
//                   {paginated.map((r, i) => {
//                     const sev = Math.abs(r.anomaly_score) > 0.2 ? 'Très suspect' : Math.abs(r.anomaly_score) > 0.1 ? 'Suspect' : 'À vérifier';
//                     const color = Math.abs(r.anomaly_score) > 0.2 ? '#A32D2D' : Math.abs(r.anomaly_score) > 0.1 ? '#854F0B' : '#666';
//                     const bg = Math.abs(r.anomaly_score) > 0.2 ? '#FFF5F5' : Math.abs(r.anomaly_score) > 0.1 ? '#FFFBEB' : '#F8F8F8';
//                     const border = Math.abs(r.anomaly_score) > 0.2 ? '#FCA5A5' : Math.abs(r.anomaly_score) > 0.1 ? '#FCD34D' : '#e0e0e0';
//                     return (
//                       <div key={i} className="rounded-xl border p-4 flex flex-col gap-2" style={{ background: bg, borderColor: border }}>
//                         <div className="flex items-center justify-between gap-2 flex-wrap">
//                           <span className="font-mono text-[0.75rem] font-semibold text-[#0b7db0] bg-white border border-[#B5D4F4] rounded px-2 py-0.5 flex-shrink-0">{r.AR_Ref}</span>
//                           <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full flex-shrink-0" style={{ background: color + '18', color }}>{sev}</span>
//                         </div>
//                         <p className="text-[0.8rem] text-[#444] m-0 font-medium truncate" title={getDesign(r)}>{getDesign(r)}</p>
//                         <p className="text-[10px] text-[#888] m-0">{fmtDate(r.DateJour)}</p>
//                         <div className="grid grid-cols-2 gap-2 mt-1">
//                           <div className="bg-white rounded-lg px-3 py-2 border border-[#f0f0f0]">
//                             <p className="text-[10px] text-[#aaa] m-0">Quantité sortie</p>
//                             <p className="text-[0.9rem] font-bold m-0" style={{ color }}>{fmtNum(r.TotalSortie)} u.</p>
//                           </div>
//                           <div className="bg-white rounded-lg px-3 py-2 border border-[#f0f0f0]">
//                             <p className="text-[10px] text-[#aaa] m-0">Stock restant</p>
//                             <p className="text-[0.9rem] font-bold m-0 text-[#555]">{fmtNum(r.StockFinal)} u.</p>
//                           </div>
//                         </div>
//                       </div>
//                     );
...
// // ── Onglets config ────────────────────────────────────────────
// const TABS = [
//   { id: 'commandes',    label: 'Commandes urgentes',    icon: ShoppingCart, color: '#EF4444' },
//   { id: 'previsions',   label: 'Prévisions du mois',    icon: PackageCheck, color: '#0EA5E9' },
//   { id: 'anomalies',    label: 'Mouvements suspects',   icon: Radar,        color: '#8B5CF6' },
//   { id: 'prioritaires', label: 'Articles prioritaires', icon: Star,         color: '#F59E0B' },
// ];
...
function ResumeBandeau({ critiques, alertes, suspects, prioritaires, baseName }) {
  const genererPhrase = () => {
    ...
    if (critiques > 0) parties.push(`${critiques} article${critiques > 1 ? 's' : ''} à commander d'urgence`);
    if (alertes > 0) parties.push(`${alertes} à planifier cette semaine`);
    if (suspects > 0) parties.push(`${suspects} article${suspects > 1 ? 's ont' : ' a'} des mouvements à vérifier`);
    return `Aujourd'hui pour ${baseName} : ${parties.join(', ')}.`;
  };
...
export default function PageAlertes() {
  ...
  // ── Calculs pour le bandeau résumé (basés sur les mêmes règles que les onglets) ──
  // IMPORTANT : "suspects" compte les ARTICLES DISTINCTS, pas les lignes brutes.
  // Un même article peut avoir plusieurs mouvements suspects à des dates différentes ;
  // pour un employé, ce qui compte c'est combien d'articles différents aller vérifier,
  // pas combien de lignes existent dans l'historique.
  const resumeData = useMemo(() => {
    const rupturesSorted = ruptures.filter(r => !isNaN(Number(r.jours_estimes)) && Number(r.jours_estimes) > 0);
    const critiques = rupturesSorted.filter(r => Number(r.jours_estimes) < 7).length;
    const alertesCount = rupturesSorted.filter(r => Number(r.jours_estimes) >= 7 && Number(r.jours_estimes) < 30).length;

    const lignesSuspectes = anomalies.filter(r => r.is_anomalie === 1);
    const suspects = new Set(lignesSuspectes.map(r => r.AR_Ref)).size;
