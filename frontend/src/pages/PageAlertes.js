...
// < 1 jour  → uniquement en heures (ex: "5 h")
// >= 1 jour → jours entiers + heures restantes (ex: "2 j 15 h")
...
function TabAnomalies({ anomalies, baseName }) {
  ...
  const paginated = severes.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);
      ...
      <div className="bg-white border border-[#e8e8e8] rounded-2xl overflow-hidden shadow-[0_1px_6px_rgba(0,0,0,0.06)]">
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-[#f0f0f0]">
          <div className="flex items-center gap-3">
            ...
            <span className="bg-[#f0f0f0] text-[#666666] text-[0.6875rem] font-mono px-2 py-0.5 rounded">{severes.length} mouvements</span>
          </div>
          {severes.length > 0 && (
            <ExportButton onClick={() => exportCSV(severes, `anomalies_${baseName}.csv`,
        ...
        {severes.length === 0 ? (
          ...
          <>
            ...
            <Pagination page={page} total={severes.length} pageSize={PAGE_SIZE} onChange={setPage} />
          </>
        )}
      </div>
    </div>
